...
import { javaCourseData } from "@/lib/java-course-data"
import LessonContent from "@/components/lesson-content"
import ModuleQuiz from "@/components/module-quiz"
import FinalExam from "@/components/final-exam"
import Certificate from "@/components/java/certificate"

export default function JavaCoursePlatform() {
  const [selectedModule, setSelectedModule] = useState(0)
  const [selectedLesson, setSelectedLesson] = useState(0)
  const [activeTab, setActiveTab] = useState("lesson")
  const [expandedModules, setExpandedModules] = useState<Set<number>>(
    new Set(Array.from({ length: javaCourseData.modules.length }, (_, i) => i)),
  )
  const [completedLessons, setCompletedLessons] = useState<Set<string>>(
    new Set(
      javaCourseData.modules.flatMap((module, moduleIndex) =>
        module.lessons.map((_, lessonIndex) => `${moduleIndex}-${lessonIndex}`),
      ),
    ),
  )
  const [completedQuizzes, setCompletedQuizzes] = useState<Set<number>>(new Set())
  const [examCompleted, setExamCompleted] = useState(true) // Set exam as completed
  const [showCertificate, setShowCertificate] = useState(false)
  const [showCertificatePreview, setShowCertificatePreview] = useState(false)

  const totalLessons = javaCourseData.modules.reduce((acc, module) => acc + module.lessons.length, 0)
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-balance">Java Mastery</h1>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-2xl font-bold text-orange-600">{Math.round(progress)}%</p>
              </div>
              <Button
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6 p-6 rounded-xl bg-gradient-to-r from-orange-500/10 to-red-600/10 border-2 border-orange-500/30">
          <h2 className="text-3xl font-bold mb-2 text-balance">Java Professional Mastery Course</h2>
          <p className="text-muted-foreground mb-4">
            Master Java from fundamentals to advanced OOP concepts. Learn exception handling, collections framework, and
            build professional applications. Complete all modules, pass the final exam, and earn your certificate!
          </p>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-orange-600">
            ...
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-orange-600">
              ...
              <div>
                ...
                <p className="font-bold">{javaCourseData.modules.length} Total</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <CardContent className="p-0">
                <ScrollArea className="h-[calc(100vh-280px)]">
                  <div className="space-y-1 p-4">
                    {javaCourseData.modules.map((module, moduleIndex) => (
                      <div key={moduleIndex} className="space-y-1">
                        <button
                          onClick={() => toggleModule(moduleIndex)}
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedModule === moduleIndex
                              ? "bg-orange-600 text-white shadow-md border-l-4 border-orange-800"
                              : "hover:bg-orange-500/10 hover:border-orange-500/30 border border-transparent"
                          }`}
                        >
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <Progress
                              ...
                              className="h-1 [&>div]:bg-orange-800"
                            />
                            ...
                            {module.lessons.map((lesson, lessonIndex) => (
                              <button
                                ...
                                className={`w-full text-left p-2 rounded-md text-sm flex items-center gap-2 transition-colors ${
                                  ...
                                  activeTab === "lesson"
                                    ? "bg-orange-500/20 font-medium border-l-2 border-orange-600"
                    ...
                    <div className="border-t pt-4 mt-4">
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-lg transition-colors border-2 ${
                          activeTab === "exam"
                            ? "bg-orange-600 text-white border-orange-800 shadow-md"
                            : progress >= 100
                      ...
                      >
                        ...
                        {progress < 100 && (
                          <Badge className="mt-2 bg-orange-500/20 text-orange-600 border-orange-600">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          Lesson {selectedLesson + 1} of {javaCourseData.modules[selectedModule].lessons.length}
                        </span>
                      </div>
                      <CardTitle className="text-2xl text-balance">
                        {javaCourseData.modules[selectedModule].lessons[selectedLesson].title}
                      </CardTitle>
                      <CardDescription className="mt-2">
                        {javaCourseData.modules[selectedModule].lessons[selectedLesson].description}
                      </CardDescription>
                    </div>
                ...
                <CardContent>
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="lesson">
                      <LessonContent
                        lesson={javaCourseData.modules[selectedModule].lessons[selectedLesson]}
                      ...
                      <div className="space-y-4">
                        <div className="aspect-video bg-muted rounded-lg overflow-hidden">
                          <iframe
                            width="100%"
                            height="100%"
                            src={javaCourseData.modules[selectedModule].videoUrl}
                        ...
                        <div className="prose prose-sm dark:prose-invert max-w-none">
                          ...
                          <p>{javaCourseData.modules[selectedModule].videoDescription}</p>
                        </div>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="quiz">
                      <ModuleQuiz
                        module={javaCourseData.modules[selectedModule]}
