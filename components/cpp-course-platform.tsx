...
import { CheckCircle2, Circle, PlayCircle, BookOpen, Award, ChevronRight, Code2, Eye, Lock, X, Home, Menu } from 'lucide-react'
import { cppCourseData } from "@/lib/cpp-course-data"
import CPPLessonContent from "@/components/cpp-lesson-content"
import CPPModuleQuiz from "@/components/cpp-module-quiz"
import CPPFinalExam from "@/components/cpp-final-exam"
import CPPCertificate from "@/components/cpp-certificate"
import CPPExamplePracticeExam from "@/components/cpp-example-practice-exam"
...
export default function CPPCoursePlatform() {
  ...
  if (showCertificate) {
    return <CPPCertificate onClose={() => setShowCertificate(false)} progress={progress} />
  }

  if (showCertificatePreview) {
    return <CPPCertificate onClose={() => setShowCertificatePreview(false)} progress={progress} preview={true} />
  }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-6">
          <CPPExamplePracticeExam onClose={() => setShowExampleExam(false)} />
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card sticky top-0 z-10 shadow-sm">
        <div className="container mx-auto px-4 py-4">
              ...
              <div className="relative">
                ...
                {showCourseMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-card border rounded-lg shadow-lg py-2 z-50">
                    <Link href="/html-course">
                      <button className="w-full text-left px-4 py-2 hover:bg-primary/10 transition-colors">
                        <div className="font-semibold">HTML Mastery</div>
                        <div className="text-xs text-muted-foreground">Switch to HTML</div>
                      </button>
                    </Link>
                  </div>
                )}
          ...
          <div className="mt-4">
            <Progress value={progress} className={`h-2 ${progress >= 75 ? '[&>div]:bg-secondary' : '[&>div]:bg-primary'}`} />
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6 p-6 rounded-xl bg-gradient-to-r from-primary/10 via-secondary/5 to-secondary/10 border-2 border-primary/20">
          <h2 className="text-3xl font-bold mb-2 text-balance">C++ Programming Course</h2>
          <p className="text-muted-foreground mb-4">
            Master C++ from fundamentals to advanced techniques. Learn OOP, STL, memory management, and modern C++ features to build powerful applications.
          </p>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            {activeTab === "exam" ? (
              <CPPFinalExam onComplete={() => setExamCompleted(true)} completed={examCompleted} />
            ) : (
              <Card>
                <CardHeader>
