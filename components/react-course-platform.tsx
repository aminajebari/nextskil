...
import { reactCourseData } from "@/lib/react-course-data"
...
export default function ReactCoursePlatform() {
  const [selectedModule, setSelectedModule] = useState(0)
  const [selectedLesson, setSelectedLesson] = useState(0)
  const [activeTab, setActiveTab] = useState("lesson")
  const [expandedModules, setExpandedModules] = useState<Set<number>>(
    new Set(Array.from({ length: reactCourseData.modules.length }, (_, i) => i)),
  )
  const [completedLessons, setCompletedLessons] = useState<Set<string>>(
    new Set(
      reactCourseData.modules.flatMap((module, moduleIndex) =>
  ...
  const totalLessons = reactCourseData.modules.reduce((acc, module) => acc + module.lessons.length, 0)
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-cyan-600 to-blue-600 flex items-center justify-center shadow-md">
                <Code2 className="h-7 w-7 text-white" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-balance">React Fundamentals</h1>
                <p className="text-sm text-muted-foreground">Build Modern User Interfaces</p>
              </div>
            </div>
          ...
          <div className="mt-4">
            <Progress value={progress} className="h-2 [&>div]:bg-cyan-600" />
            <div className="flex items-center justify-between mt-2 text-sm">
              <span className="text-muted-foreground">
                {completedLessons.size} of {totalLessons} lessons
              </span>
              <span className="font-semibold text-cyan-600">{Math.round(progress)}% Complete</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6 p-6 rounded-xl bg-gradient-to-r from-cyan-600/10 to-blue-600/10 border-2 border-cyan-600/30">
          <h2 className="text-3xl font-bold mb-2 text-balance">React Fundamentals Course</h2>
          <p className="text-muted-foreground mb-4">
            Master React from the ground up. Learn components, state management, hooks, and build modern web
            applications. Complete all modules, pass the final exam, and earn your certificate!
          </p>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-cyan-600">
            ...
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-cyan-600">
              ...
              <div>
                ...
                <p className="font-bold">{reactCourseData.modules.length} Total</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1">
            <Card>
              <CardHeader className="bg-cyan-600/5 border-b border-cyan-600/20">
              ...
              <CardContent className="p-0">
                <ScrollArea className="h-[calc(100vh-280px)]">
                  <div className="space-y-1 p-4">
                    {reactCourseData.modules.map((module, moduleIndex) => (
                      <div key={moduleIndex} className="space-y-1">
                        <button
                          onClick={() => toggleModule(moduleIndex)}
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedModule === moduleIndex
                              ? "bg-cyan-600 text-white shadow-md border-l-4 border-blue-600"
                              : "hover:bg-cyan-600/10 hover:border-cyan-600/30 border border-transparent"
                          }`}
                        >
                            ...
                            {module.lessons.map((lesson, lessonIndex) => (
                              <button
                                ...
                                className={`w-full text-left p-2 rounded-md text-sm flex items-center gap-2 transition-colors ${
                                  ...
                                  activeTab === "lesson"
                                    ? "bg-cyan-600/20 font-medium border-l-2 border-cyan-600"
                    ...
                    <div className="border-t pt-4 mt-4">
                      <button
                        ...
                        className="w-full text-left p-3 rounded-lg transition-colors bg-cyan-600/5 hover:bg-cyan-600/10 border-2 border-cyan-600/30 mb-2"
                      >
                      ...
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-lg transition-colors border-2 ${
                          activeTab === "exam"
                            ...
                            : progress >= 100
                              ? "bg-cyan-600/5 hover:bg-cyan-600/10 border-cyan-600/30"
                      ...
                      >
                        ...
                        {progress < 100 && (
                          <Badge className="mt-2 bg-cyan-600/20 text-cyan-600 border-cyan-600">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          Lesson {selectedLesson + 1} of {reactCourseData.modules[selectedModule].lessons.length}
                        </span>
                      </div>
                      <CardTitle className="text-2xl text-balance">
                        {reactCourseData.modules[selectedModule].lessons[selectedLesson].title}
                      </CardTitle>
                      <CardDescription className="mt-2">
                        {reactCourseData.modules[selectedModule].lessons[selectedLesson].description}
                      </CardDescription>
                    </div>
                ...
                <CardContent>
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="lesson">
                      <LessonContent
                        lesson={reactCourseData.modules[selectedModule].lessons[selectedLesson]}
                      ...
                      <div className="space-y-4">
                        <div className="aspect-video bg-muted rounded-lg overflow-hidden">
                          <iframe
                            width="100%"
                            height="100%"
                            src={reactCourseData.modules[selectedModule].videoUrl}
                        ...
                        <div className="prose prose-sm dark:prose-invert max-w-none">
                          ...
                          <p>{reactCourseData.modules[selectedModule].videoDescription}</p>
                        </div>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="quiz">
                      <ModuleQuiz
                        module={reactCourseData.modules[selectedModule]}
