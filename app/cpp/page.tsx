...
import { CheckCircle2, Circle, PlayCircle, BookOpen, Award, ChevronRight, Code2, Eye, Lock, X } from 'lucide-react'
import { cppCourseData } from "../../lib/cpp-course-data"
import { cppFinalExamQuestions } from "../../lib/cpp-final-exam-data"
...
export default function CppCoursePlatform() {
  const [selectedModule, setSelectedModule] = useState(0)
  const [selectedLesson, setSelectedLesson] = useState(0)
  const [activeTab, setActiveTab] = useState("lesson")
  const [expandedModules, setExpandedModules] = useState<Set<number>>(
    new Set(Array.from({ length: cppCourseData.modules.length }, (_, i) => i))
  )
  ...
  const totalLessons = cppCourseData.modules.reduce((acc, module) => acc + module.lessons.length, 0)
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card sticky top-0 z-10 shadow-sm">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-primary to-red-500 flex items-center justify-center shadow-md">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-2xl font-bold text-red-500">{Math.round(progress)}%</p>
              </div>
              <Button
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6 p-6 rounded-xl bg-gradient-to-r from-primary/10 to-red-500/10 border-2 border-red-500/30">
          <h2 className="text-3xl font-bold mb-2 text-balance">C++ Professional Mastery Course</h2>
          <p className="text-muted-foreground mb-4">
            Master C++ from fundamentals to advanced techniques. Learn OOP, memory management, STL, and build professional applications. Pass all modules, complete the final exam, and earn your certificate!
          </p>
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-red-500">
              <BookOpen className="h-5 w-5 text-red-500" />
              <div>
                <p className="text-xs text-muted-foreground">Completed</p>
                <p className="font-bold">{completedLessons.size}/{totalLessons} Lessons</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-background/80 px-4 py-2 rounded-lg border-l-4 border-red-500">
              ...
              <div>
                ...
                <p className="font-bold">{cppCourseData.modules.length} Total</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1">
            <Card>
              <CardHeader className="bg-primary/5 border-b border-red-500/20">
              ...
              <CardContent className="p-0">
                <ScrollArea className="h-[calc(100vh-280px)]">
                  <div className="space-y-1 p-4">
                    {cppCourseData.modules.map((module, moduleIndex) => (
                      <div key={moduleIndex} className="space-y-1">
                        <button
                          onClick={() => toggleModule(moduleIndex)}
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedModule === moduleIndex 
                              ? "bg-primary text-primary-foreground shadow-md border-l-4 border-red-500" 
                            ...
                            {module.lessons.map((lesson, lessonIndex) => (
                              <button
                                ...
                                className={`w-full text-left p-2 rounded-md text-sm flex items-center gap-2 transition-colors ${
                                  selectedModule === moduleIndex && selectedLesson === lessonIndex && activeTab === "lesson"
                                    ? "bg-red-500/20 font-medium border-l-2 border-red-500"
                    ...
                    <div className="border-t pt-4 mt-4">
                      <button
                        ...
                        className="w-full text-left p-3 rounded-lg transition-colors bg-red-500/5 hover:bg-red-500/10 border-2 border-red-500/30 mb-2"
                      >
                      ...
                      >
                        ...
                        {progress < 100 && (
                          <Badge className="mt-2 bg-red-500/20 text-red-700 border-red-500">
                            Locked - {Math.round(progress)}% Complete
                          </Badge>
                        )}
                        {examCompleted && <Badge className="mt-2 bg-green-500 text-white">Completed ✓</Badge>}
                      </button>
                    </div>
                  </div>
                </ScrollArea>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          Lesson {selectedLesson + 1} of {cppCourseData.modules[selectedModule].lessons.length}
                        </span>
                      </div>
                      <CardTitle className="text-2xl text-balance">
                        {cppCourseData.modules[selectedModule].lessons[selectedLesson].title}
                      </CardTitle>
                      <CardDescription className="mt-2">
                        {cppCourseData.modules[selectedModule].lessons[selectedLesson].description}
                      </CardDescription>
                    </div>
                ...
                <CardContent>
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="lesson">
                      <LessonContent
                        lesson={cppCourseData.modules[selectedModule].lessons[selectedLesson]}
                      ...
                      <div className="space-y-4">
                        <div className="aspect-video bg-muted rounded-lg overflow-hidden">
                          <iframe
                            width="100%"
                            height="100%"
                            src={cppCourseData.modules[selectedModule].videoUrl}
                        ...
                        <div className="prose prose-sm dark:prose-invert max-w-none">
                          ...
                          <p>{cppCourseData.modules[selectedModule].videoDescription}</p>
                        </div>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="quiz">
                      <ModuleQuiz
                        module={cppCourseData.modules[selectedModule]}
